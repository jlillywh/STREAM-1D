...
pub fn solve_unsteady_step(
    ...
    for i in 0..n - 1 {
        ...
        let dk_dy_i = structure_coupling::compute_dk_dy(&tables[i], &xs_list[i], y_current[i]);
        let dk_dy_ip = structure_coupling::compute_dk_dy(&tables[i + 1], &xs_list[i + 1], y_current[i + 1]);
