...
pub(crate) fn steady_result_json(inputs_json: &str) -> Result<String, String> {
    let inputs: crate::solvers::SteadyInputs = serde_json::from_str(inputs_json)
        .map_err(|e| format!("Failed to parse SteadyInputs JSON: {}", e))?;
...
pub(crate) fn culvert_rating_curve_json(inputs_json: &str) -> Result<String, String> {
    ...
    serde_json::to_string(&result)
        .map_err(|e| format!("Failed to serialize CulvertRatingCurveResult: {}", e))
}
...
pub(crate) fn bridge_rating_curve_json(inputs_json: &str) -> Result<String, String> {
    ...
    serde_json::to_string(&result)
        .map_err(|e| format!("Failed to serialize BridgeRatingCurveResult: {}", e))
}
...
mod tests {
    ...
    fn json_helpers_roundtrip() {
        ...
        let steady_result: crate::solvers::SteadyResult =
            serde_json::from_str(&steady_out).unwrap();
        ...
        assert!(culvert_rating_curve_json(culvert_json)
            .unwrap()
            .contains("\"q\""));
        ...
        assert!(bridge_rating_curve_json(bridge_json)
            .unwrap()
            .contains("flow_regimes"));
    }
