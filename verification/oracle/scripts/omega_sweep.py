...
def run_verify():
    ...
    p = subprocess.run(
        ["python3", "verification/oracle/run_linked_verify.py", "--scenario", str(SCENARIO), "--format", "matrix"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
...
for o in OMEGAS:
    ...
    results.append(
        {
            "omega": o,
            "build_tail": b.stdout.strip().splitlines()[-1] if b.stdout.strip() else "",
            "overall_line": ol,
            "overall_val": ov,
            "rm_max_delta": {r: parse_rm(out, r) for r in RMS},
        }
    )
...
(ROOT / "verification/oracle/omega_sweep_summary.json").write_text(
    json.dumps({"results": results, "chosen_omega": best, "top3": valid[:3]}, indent=2)
)
print("CHOSEN", best)
for r in results:
    print(r["omega"], r["overall_val"], r["rm_max_delta"])
