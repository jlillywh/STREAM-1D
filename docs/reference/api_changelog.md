...
JSON/Python input schema version (`api_version` in WASM metadata). Current version: **33**.

| Version | Change |
|---------|--------|
| 33 | Unsteady Preissmann structure coupling stub (Phase 5.2): `unsteady_structure_coupling_mode` (`0` = post-step only default, `2` = implicit Jacobian opt-in); WASM metadata `unsteady_structure_coupling_modes`. Mode `2` is wired but physics-neutral until Phase 3 implicit residuals land — see [`unsteady_implicit_bridge_coupling.md`](../development/unsteady_implicit_bridge_coupling.md) |
