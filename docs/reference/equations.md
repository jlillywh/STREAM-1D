...
* **Hybrid Preissmann structure coupling (API v33+, mode `2`):** One Preissmann solve per step replaces the reach momentum row at eligible structure intervals with implicit headwater residuals (stages only; section $Q$ held). **Culvert:** inlet-controlled circular/box, single barrel, no overtopping. **Bridge:** subcritical low-flow (`low_a` / `low_b`) with regime pinning; high-flow, supercritical (`low_c`), and pressure/weir intervals defer to explicit post-step `solve_bridge_coupled` / `converge_culvert_headwater` that step. Mode `1` (reach–structure–reach outer loop) is reserved and not implemented. Design: [`unsteady_implicit_bridge_coupling.md`](../development/unsteady_implicit_bridge_coupling.md).
