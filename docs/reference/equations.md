...
* **Preissmann structure coupling (API v33):** `unsteady_structure_coupling_mode` selects how inline structures participate in each Preissmann step: `0` (default) = reach-only Jacobian with explicit post-step structure passes; `2` = implicit Jacobian hook at tagged structure intervals (opt-in). Mode `2` embeds **inlet-controlled circular/box culvert** FHWA headwater residuals (stages only; section $Q$ held from Preissmann); outlet control, overtopping, multi-barrel, and bridges still use explicit post-step until extended. See [`unsteady_implicit_bridge_coupling.md`](../development/unsteady_implicit_bridge_coupling.md).
