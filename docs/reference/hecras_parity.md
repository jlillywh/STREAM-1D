...
| Nose shape (square, semicircular, …) for Yarnell $K$ / drag $C_D$ | `bridge_pier_shapes` — **one enum per bridge**, not per pier | **Partial** — coefficients match HEC-RAS table; cannot mix shapes per pier in one opening |
