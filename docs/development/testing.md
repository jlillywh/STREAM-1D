...
| **Reverse-flow rating (v31, §4.3)** | `tests/bridge_reverse_flow_rating_verification.rs`, `src/solvers/bridge_tests.rs` (`test_bridge_rating_curve_bidirectional_qmax_sweep`, `test_bridge_rating_curve_negative_q_*`, `test_bridge_rating_curve_mixed_sign_q_values`), `tests/wasm_json_contract.rs` (`wasm_bridge_rating_curve_negative_q_contract`) | [`verification/fixtures/bridge_reverse_flow_rating.json`](../verification/fixtures/bridge_reverse_flow_rating.json) — $Q \in [-Q_{max}, +Q_{max}]$; per-direction HW monotonicity; $\pm|Q|$ head-loss parity |
